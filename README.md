...
1.  **Memorização Ativa e Repetição Espaçada:** Implementação do consagrado algoritmo **SuperMemo-2 (SM-2)** para agendar automaticamente as revisões de flashcards baseando-se no feedback de facilidade do usuário.
...
## 🌐 4. MVP / Protótipo Online e Instruções de Uso
...
### 🚀 Instruções de Teste Rápido (para a Banca Avaliadora)

Para avaliar o MVP sem instalar absolutamente nada no seu dispositivo, siga os passos abaixo:

1.  **Acesse o portal:** Entre no link do frontend: [https://saafo.tiezar.pro](https://saafo.tiezar.pro).
2.  **Criar Conta ou Acessar:**
    *   Clique em **Registrar** e insira seu e-mail e senha, ou faça login de forma rápida usando sua conta Google através do botão **Google OAuth** (caso configurado).
    *   *Alternativamente, você pode usar os seguintes dados de teste:*
        *   **Usuário de Teste:** `banca@saafo.pro` *(Preencha aqui caso queira cadastrar um usuário padrão no banco e expor a senha)*
        *   **Senha:** `SaafoTeste123`
3.  **Criação de Matéria e Tópicos:**
    *   No menu lateral, clique em **Matérias (Materials)**.
    *   Crie uma nova matéria (Ex: "Engenharia de Software") e defina uma cor identificadora.
    *   Dentro da matéria criada, adicione um tópico de estudo (Ex: "Requisitos de Software").
4.  **Geração de Flashcards por IA:**
    *   Navegue até a seção de **Gerador de IA (AIGenerator)**.
    *   Selecione o Tópico criado e digite um texto resumido de aula ou faça o upload de um documento PDF/Imagem (até 50MB).
    *   Clique em **Gerar por IA** e assista a inteligência artificial formular instantaneamente uma lista estruturada de cartões de estudo (frente/verso).
5.  **Modo de Revisão (SM-2):**
    *   Inicie uma sessão de estudos. O player será aberto em tela cheia com os cartões ativos.
    *   Leia a pergunta, tente lembrar a resposta e pressione a **Barra de Espaço** (ou clique na tela) para revelar o verso.
    *   Classifique seu grau de memorização de 0 a 5 clicando nos botões ou usando os atalhos numéricos do teclado (`1` a `5`). O algoritmo SM-2 recalculará a próxima data de revisão de forma transparente.
6.  **Simulação de Prova:**
    ...
    *   Responda as questões e clique em enviar para receber um feedback com notas e correções contextualizadas da IA.
...
|:---:|:---:|
| ![Dashboard](./frontend/public/screenshots/dashboard.png) | ![Materiais](./frontend/public/screenshots/materials.png) |
...
*   **O que a equipe revisou, adaptou ou validou:** A equipe Front-Enzos analisou e validou todas as sugestões sugeradas, adaptando-as conforme necessário para garantir a integridade do algoritmo SuperMemo-2 (SM-2) de repetição espaçada e a aderência aos requisitos especificados.

---

## 🎥 10. Apresentação e Pitch do Projeto

*   **🔗 Link dos Slides da Apresentação:** **`[ADICIONE O LINK DO SLIDESHARE / GOOGLE SLIDES AQUI]`** ⚠️
*   **🔗 Link do Vídeo de Pitch (YouTube / Drive):** **`[ADICIONE O LINK DO VÍDEO DO PITCH AQUI]`** ⚠️

---

## 🏗️ 11. Arquitetura MTC (Exigência Acadêmica)
...
*Desenvolvido com carinho e compromisso acadêmico pela equipe **Front-Enzos**.* 🚀
