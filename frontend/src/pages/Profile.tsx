...
export default function Profile() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'conta' | 'assinatura' | 'rotina'>('conta');
  ...
  return (
    <div className="page" style={{ padding: isMobile ? '16px 12px 120px' : '24px 24px 48px' }}>
      <DeleteAccountModal
      ...
      />
      {/* Page Header */}
      <header style={{ marginBottom: 32, borderBottom: '1px solid var(--border-color)', paddingBottom: 20 }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: isMobile ? 26 : 36, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>Perfil</h2>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 16, color: 'var(--text-muted)', marginTop: 8 }}>
          Gerencie suas informações pessoais, rotinas e detalhes da sua assinatura.
        </p>
      </header>
      ...
      {activeSubTab === 'conta' && (
        <div style={{ maxWidth: 600, display: 'flex', flexDirection: 'column', gap: 48 }}>
          <div>
            ...
            <form onSubmit={save} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div>
                ...
                />
                <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6, lineHeight: 1.5, display: 'flex', alignItems: 'flex-start', gap: 6 }}>
          ...
          {/* Danger Zone */}
          <div style={{ marginTop: 24, borderTop: '1px solid var(--border-color)', paddingTop: 32 }}>
      ...
      {activeSubTab === 'assinatura' && (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(320px, 1fr))', gap: isMobile ? 24 : 48 }}>
          {/* Subscription Panel */}
              ...
              {weeklyRoutines.map(r => (
                <div key={r.id} style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', padding: 20, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6 }}>
                      <Calendar size={13} />
